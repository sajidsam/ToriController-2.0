...
const MainCenterView = ({ pitch = 0, roll = 0, heading = 0, speedKnots = 0, frontFinAngle = 0, rearFinX = 0, rearFinY = 0, cameraUrl }) => {
  ...
  detectRef.current = async () => {
    ...
    try {
      ...
      if (e.name !== 'SecurityError') console.error("Detection error:", e);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center bg-transparent p-4 relative overflow-hidden h-full gap-4">
        ...
        <div className="relative z-10 flex flex-wrap gap-4 lg:gap-8 w-full max-w-4xl justify-center mt-4">
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/20 p-4 rounded-xl flex-1 flex items-center justify-between gap-6 shadow-xl">
                <div className="flex flex-col gap-1 items-center justify-between  bg-white/5 p-3 rounded-lg border border-white/10 min-w-[80px]">
                    <span className="text-[10px] text-white/50 font-bold uppercase tracking-widest">Pitch</span>
                    <span className="text-2xl font-mono text-white font-bold flex items-center gap-1 drop-shadow-sm">
                ...
                <div className="flex flex-col gap-1 items-center bg-white/5 p-3 rounded-lg border border-white/10 min-w-[80px]">
            ...
            {/* Speed Indicator */}
            <div className="bg-black/60 backdrop-blur-md border border-white/20 p-4 rounded-xl flex items-center justify-between shadow-xl gap-4">
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/20 p-4 rounded-xl flex items-center justify-between shadow-xl gap-4">
                ...
                <div className="w-12 h-12 rounded-full border-2 border-white/20 bg-white/5 flex items-center justify-center relative shadow-inner">
                    <Navigation
                        size={20}
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/20 p-3 rounded-xl flex flex-col justify-center shadow-xl gap-1">
                ...
                <div className="flex justify-between gap-4 text-xs font-mono">
                    ...
                    <span className={`${frontFinAngle === 0 ? 'text-white/50' : 'text-white font-bold'}`}>{frontFinAngle > 0 ? '+' : ''}{frontFinAngle}°</span>
                </div>
                <div className="flex justify-between gap-4 text-xs font-mono">
                    <span className="text-white/50">YAW:</span>
                    <span className={`${rearFinX === 0 ? 'text-white/50' : 'text-white font-bold'}`}>{rearFinX > 0 ? '+' : ''}{rearFinX}°</span>
                </div>
                <div className="flex justify-between gap-4 text-xs font-mono">
                    <span className="text-white/50">PTCH:</span>
                    <span className={`${rearFinY === 0 ? 'text-white/50' : 'text-white font-bold'}`}>{rearFinY > 0 ? '+' : ''}{rearFinY}°</span>
                </div>
            </div>
