...
const MainCenterView = ({
  ...
  useEffect(() => {
    ...
    const worker = new Worker(new URL('../workers/yoloWorker.js', import.meta.url), { type: 'module' });
    workerRef.current = worker;
    
    worker.onmessage = (e) => {
      if (e.data.type === "ready") {
        setAiStatus("AI READY (OFF)");
      } else if (e.data.type === "error") {
        isInferencing.current = false;
        setAiStatus("ERROR: " + e.data.message);
        console.error("Worker Error:", e.data.message);
      } else if (e.data.type === "result") {
        isInferencing.current = false;
        handleDetectionResult(e.data.boxes, e.data.width, e.data.height);
      }
    };
  ...
  const handleDetectionResult = (predictions, canvasWidth, canvasHeight) => {
    ...
    predictions.forEach((prediction) => {
      const x = prediction.box[0] * scaleX;
      const y = prediction.box[1] * scaleY;
      const width = prediction.box[2] * scaleX;
      const height = prediction.box[3] * scaleY;
      
      const text = `${prediction.className} (${(prediction.score * 100).toFixed(0)}%)`;
      const isHighAccuracy = prediction.score > 0.75;
      const color = isHighAccuracy ? "#ef4444" : "#22c55e";
    ...
    predictions.forEach((p) => {
      if (!classScores[p.className] || p.score > classScores[p.className]) {
        classScores[p.className] = p.score;
      }
    });
    
    const uniqueClassesWithScores = Object.entries(classScores)
      .map(([cls, score]) => ({ class: cls, score }))
      .sort((a, b) => a.class.localeCompare(b.class));
      
    setDetectedObjects((prev) => {
      const newStr = uniqueClassesWithScores
        .map((c) => `${c.class}:${c.score > 0.75}`)
        .join(",");
      const oldStr = (prev || [])
        .map((c) => `${c.class}:${c.score > 0.75}`)
        .join(",");
  ...
  detectRef.current = () => {
    if (!workerRef.current || !imgRef.current || !canvasRef.current) return;
    if (isInferencing.current) return;

    if (!isAiEnabled) {
      if (aiStatus !== "DISABLED" && aiStatus !== "AI READY (OFF)") {
        setAiStatus("AI READY (OFF)");
        setDetectedObjects([]);
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      if (isRunning) {
        ...
        }, 100); // 10 fps throttle since worker is async
      }
    };
        ...
        >
          ...
          {!isImageLoaded && (
            <div className="text-white/85 flex flex-col items-center gap-2 absolute z-0 mt-32 w-full text-center select-none pointer-events-none">
              <span className="text-xs font-mono tracking-widest font-bold opacity-80">
                NO VIDEO SIGNAL - FALLBACK TO HUD
              </span>
            </div>
          )}
